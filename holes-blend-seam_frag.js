export const fragmentShader = `
    ...
    void main() {
        float seamBlend = 10.1; // Adjust this value to control the blending width around the seam
        float seamPosition = 1.0 - seamBlend;
    
        vec3 P = vec3(vUv * 50.0, time * 0.5);
    
        float noiseValue = noise(P);
    
        if (vUv.x > seamPosition) {
            float t = (vUv.x - seamPosition) / seamBlend;
            vec3 P2 = vec3(vec2(vUv.x - 1.0) * 50.0, time * 0.5);
        ...
        gl_FragColor = vec4(82.0, 151.0, 70.0, holeMask);
    }
`
